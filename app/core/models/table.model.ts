...
export class AppTable {
  ...
  sortBy: string = '';
  sortOrder: string = '';
  rowsPerPageOptions: Array<number> = [25, 50, 100, 500, 1000];
  ...
  getTableParams = () => {
    ...
    const filterList: { [key in string]: any } = {};
    Object.entries(this.filters).forEach(([key, data]) => {
      if (data?.value !== undefined && data?.value !== null && data?.value !== '') {
        filterList[key] = data.value;
      }
    });

    // Set Permanent Filter
    Object.entries(this.permanentFilter).forEach(([key, value]) => {
      filterList[key] = JSON.parse(JSON.stringify(value));
    });

    this.search = JSON.stringify(filterList);
