...
@Component({
  selector: 'app-rd-product',
  templateUrl: './rd-product.component.html',
  styleUrls: ['./rd-product.component.scss'],
  standalone: true,
  imports: [SharedModule]
})
export class RdProductComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'RD-PRODUCT' }]);
    this.ds.setPageTitle('RD-PRODUCT');
  ...
  getTableData() {
    ...
    this.sb.tableData = this.api.get(ApiRoutes.recurringProductData, params).subscribe((res) => {
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddRdProductComponent, {
      header: 'Add New Code',
      width: '500px',
      appendTo: 'body',
      maximizable: true
  ...
  onUpdateRecord(data: any) {
    if (data['is_locked'] != 1 && data['id']) {
      this._sharedService.setRDProductId(data['id']);
      this._router.navigateByUrl(APP_ROUTES.addRdProduct);
    }
  }
