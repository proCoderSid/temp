...
export class SavingsProductComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'SAVINGS PRODUCT ' }]);
    this.ds.setPageTitle('SAVINGS PRODUCT');
  ...
  getTableData() {
    ...
    this.sb.tableData = this.api.get(ApiRoutes.savingProductData, params).subscribe((res) => {
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddSavingsProductComponent, {
      header: 'Add New Code',
      width: '500px',
      appendTo: 'body',
      maximizable: true
  ...
  onUpdateRecord(data: any) {
    if (data['is_locked'] != 1 && data['id']) {
      this._sharedService.setSavingsProductId(data['id']);
      this._router.navigateByUrl(APP_ROUTES.addSavingProduct);
    }
  }
  ...
  onDeleteRecord(data: any) {
    ...
    this.ds.confirm.confirm({
      ...
      accept: () => {
        ...
        this.api.delete(ApiRoutes.savingProductData, data['id']).subscribe((res) => {
          console.log(res);
        });
