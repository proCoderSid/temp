...
export class LoanProductComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'LOAN-PRODUCT' }]);
    this.ds.setPageTitle('LOAN-PRODUCT');
  ...
  getTableData() {
    ...
    this.sb.tableData = this.api.get(ApiRoutes.loanProductData, params).subscribe((res) => {
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddLoanProductComponent, {
      header: 'Add New Code',
      width: '500px',
      appendTo: 'body',
      maximizable: true
  ...
  onUpdateRecord(data: any) {
    if (data['is_locked'] != 1 && data['id']) {
      this._sharedService.setLoanProductId(data['id']);
      this._router.navigateByUrl(APP_ROUTES.addLoanProduct);
    }
  }
  ...
  onDeleteRecord(data: any) {
    ...
    this.ds.confirm.confirm({
      ...
      accept: () => {
        ...
        this.api.delete(ApiRoutes.loanProductData, data['id']).subscribe((res) => {
          console.log(res);
        });
