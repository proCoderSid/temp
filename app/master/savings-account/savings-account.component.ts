...
export class SavingsAccountComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'SAVINGS ACCOUNT' }]);
    this.ds.setPageTitle('SAVINGS ACCOUNT ');
  ...
  getTableData() {
    ...
    this.sb.tableData = this.api.get(ApiRoutes.savingAccountData, params).subscribe((res) => {
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddSavingsAccountComponent, {
      header: 'Add New savings account',
      width: '500px',
      appendTo: 'body',
      maximizable: true
    });
  ...
  onUpdateRecord(data: any) {
    if (data['id']) {
      this._sharedService.setSavingsAccountId(data['id']);
      this._router.navigateByUrl(APP_ROUTES.addSavingAccount);
    }
  }
  ...
  onDeleteRecord(data: any) {
    ...
    this.ds.confirm.confirm({
      ...
      accept: () => {
        ...
        this.api.delete(ApiRoutes.savingAccountData, data['id']).subscribe((res) => {
          console.log(res);
        });
