...
export class LoanAccountComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'LOAN ACCOUNT ' }]);
    this.ds.setPageTitle('LOAN ACCOUNT ');
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddLoanAccountComponent, {
      header: 'Add New Code',
      width: '500px',
      appendTo: 'body',
      maximizable: true
  ...
  onUpdateRecord(data: any) {
    if (data['id']) {
      // this._sharedService.setLoanAccountId(loanAccountData['id']);
      this._router.navigateByUrl(APP_ROUTES.addLoanAccount);
    }
    // this.ref = this.ds.dialog.open(AddLoanAccountComponent, {
