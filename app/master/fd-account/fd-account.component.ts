...
@Component({
  selector: 'app-fd-account',
  templateUrl: './fd-account.component.html',
  styleUrls: ['./fd-account.component.scss'],
  standalone: true,
  imports: [SharedModule]
})
export class FdAccountComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'FD-ACCOUNT ' }]);
    this.ds.setPageTitle('FD ACCOUNT ');
  ...
  getTableData() {
    ...
    this.sb.tableData = this.api.get(ApiRoutes.FDAccountData, params).subscribe((res) => {
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddFdAccountComponent, {
      header: 'Add New Code',
      width: '500px',
      appendTo: 'body',
      maximizable: true
  ...
  onUpdateRecord(data: any) {
    if (data['id']) {
      this._sharedService.setFDAccountId(data['id']);
      this._router.navigateByUrl(APP_ROUTES.addFdAccount);
    }
    // this.ref = this.ds.dialog.open(AddFdAccountComponent, {
  ...
  onDeleteRecord(data: any) {
    this.ds.confirm.confirm({
      ...
      accept: () => {
        ...
        this.api.delete(ApiRoutes.FDAccountData, data['id']).subscribe((res) => {
          console.log(res);
        });
