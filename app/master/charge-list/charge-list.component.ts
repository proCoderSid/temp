...
export class ChargeListComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'CHARGES ' }]);
    this.ds.setPageTitle('CHARGES ');
  ...
  getTableData() {
    ...
    this.sb.tableData = this.api.get(ApiRoutes.chargeMaster, params).subscribe((res) => {
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddChargeComponent, {
      header: 'Add New Charges',
      width: '500px',
      appendTo: 'body',
      maximizable: true
    });
