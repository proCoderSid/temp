...
@Component({
  ...
  templateUrl: './rd-account.component.html',
  styleUrls: ['./rd-account.component.scss'],
  standalone: true,
  imports: [SharedModule]
})
export class RdAccountComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'RD ACCOUNT ' }]);
    this.ds.setPageTitle('RD ACCOUNT ');
  ...
  getTableData() {
    ...
    this.sb.tableData = this.api.get(ApiRoutes.RDAccountData, params).subscribe((res) => {
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddrdAccountComponent, {
      header: 'Add New Code',
      width: '500px',
      appendTo: 'body',
      maximizable: true
  ...
  onUpdateRecord(data: any) {
    if (data['id']) {
      this._sharedService.setRDAccountId(data['id']);
      this._router.navigateByUrl(APP_ROUTES.addRdAccount);
    }
  }
  ...
  onDeleteRecord(data: any) {
    ...
    this.ds.confirm.confirm({
      ...
      accept: () => {
        ...
        this.api.delete(ApiRoutes.RDAccountData, data['id']).subscribe((res) => {
          console.log(res);
        });
