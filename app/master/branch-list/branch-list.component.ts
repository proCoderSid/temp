...
export class BranchListComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'BRANCH' }]);
    this.ds.setPageTitle('BRANCH');
  ...
  getTableData() {
    ...
    this.sb.tableData = this.api.get(ApiRoutes.branchMaster, params).subscribe((res) => {
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddBranchListComponent, {
      header: 'Add New Code',
      width: '500px',
      appendTo: 'body',
      maximizable: true
  ...
  onUpdateRecord(data: any) {
    if (data['is_locked'] != 1 && data['id']) {
      this._sharedService.setbranchId(data['id']);
      this._router.navigateByUrl(APP_ROUTES.addBarnch);
    }
  }
  ...
  onDeleteRecord(data: any) {
    this.ds.confirm.confirm({
      ...
      accept: () => {
        ...
        this.api.delete(ApiRoutes.branchMaster, data['id']).subscribe((res) => {
          console.log(res);
        });
