...
@Component({
  selector: 'app-taluka-table',
  templateUrl: './taluka-table.component.html',
  styleUrls: ['./taluka-table.component.scss'],
  standalone: true,
  imports: [SharedModule]
})
export class TalukaTableComponent {
  ...
  ngOnInit() {
    this.ds.setBreadcrumbs([{ label: 'MASTER' }, { label: 'TALUKA' }]);
    this.ds.setPageTitle('TALUKA');
  ...
  getTableData() {
    ...
    this.sb.tableData = this.api.get(ApiRoutes.talukaMaster, params).subscribe((res) => {
  ...
  onAddRecord() {
    this.ref = this.ds.dialog.open(AddTalukaComponent, {
      header: 'Add New Taluka',
      width: '500px',
      appendTo: 'body',
      maximizable: true
    });
  ...
  onDeleteRecord(data: any) {
    ...
    this.ds.confirm.confirm({
      ...
      accept: () => {
        ...
        this.api.delete(ApiRoutes.talukaMaster, data['id']).subscribe((res) => {
          console.log(res);
        });
